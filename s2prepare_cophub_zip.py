...
def prepare_dataset(path):
    ...
    if path.suffix == '.zip':
        ...
        xmlzipfiles = [s for s in z.namelist() if "MTD_MSIL2A.xml" in s]
        if xmlzipfiles == []:
    ...
    quality_checks = root.findall('./*/Quality_Control_Checks/Quality_Inspections/quality_check')
    dicquality={}

    for check in quality_checks:
        dicquality[check.get('checkType')]=check.text
    """
    ...
    radiometric_quality_flag = root.findall('./*/Quality_Control_Checks/Quality_Inspections/RADIOMETRIC_QUALITY_FLAG')[
        0].text
        """
    sensor_quality_flag =dicquality["SENSOR_QUALITY"]  
    geometric_quality_flag= dicquality["GEOMETRIC_QUALITY"]
    general_quality_flag=dicquality["GENERAL_QUALITY"]
    format_quality_flag= dicquality["FORMAT_CORRECTNESS"]
    radiometric_quality_flag= dicquality["RADIOMETRIC_QUALITY"]
    ...
    for granule_id, images in granules.items():
        ...
        if not path.suffix == '.zip':
            ...
            img_data_path = 'zip:' + img_data_path + str(path.name)[:-4]+'.SAFE'
            print(img_data_path,'ACAAA')
        ...
        for image in images:
            ...
            for item in ten_list:
                ...
                if item in image:
                    images_ten_list.append(img_data_path+'/'+(image + ".jp2"))
            for item in twenty_list:
                if item in image:
                    images_twenty_list.append(img_data_path+'/'+(image + ".jp2"))
            for item in sixty_list:
                if item in image:
                    images_sixty_list.append(img_data_path+'/'+(image + ".jp2"))
        ...
        for image in images:
            ...
            if image[-7:] in ['B09_60m', 'B06_20m', 'B12_20m', 'B05_20m', 'SCL_20m', 'B08_10m', 'B02_60m', 'B04_10m', 'B8A_20m', 'B11_20m', 'B02_10m',
             'B8A_60m', 'B03_20m', 'B04_20m', 'B11_60m', 'B01_60m', 'B12_60m', 'B07_20m', 'B02_20m', 'B04_60m', 'B07_60m', 'B03_60m', 'B06_60m', 'B03_10m', 'B05_60m']:
...
def main(output, datasets, checksum, date):
    ...
    for dataset in datasets:
        ...
        if False: #There was something causing troubles here 
            pass
        else:
            path = Path(dataset)
            ...
            yaml_path = output_path.joinpath(path.name[:-4] + '.yaml')
